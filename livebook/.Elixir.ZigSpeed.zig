...
fn processThread(float_slice: []f32, colours: [][4]u8, start_row: usize, rows_per_thread: usize, nb_rows: usize) [][4]u8 {}
  
// / nif: generate_view/4 Threaded
pub fn generate_view(data: [*]const u8, len: u32, nb_rows: usize, imax: usize) !beam.term {
  if (len < @sizeOf(Point)) {
  ...
  const rows_per_thread = (nb_rows / cpus )

  for (0..cpus) |cpu_count| {
    const args = .{float_slice, colours, start_row, rows_per_thread, nb_rows} 
    threads[cpu_count] = try std.Threaad.spawn(.{}, processThread, args);
  }
